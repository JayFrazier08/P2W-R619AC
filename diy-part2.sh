...
set -e
# ================= 替换 ath10k-ct 为官方标准驱动 =================
echo "=== 开始递归替换 ipq40xx 架构下的 ct 驱动为标准驱动 ==="
find target/linux/ipq40xx/ -type f \( -name "Makefile" -o -name "*.mk" \) -exec sed -i 's/kmod-ath10k-ct/kmod-ath10k/g' {} +
find target/linux/ipq40xx/ -type f \( -name "Makefile" -o -name "*.mk" \) -exec sed -i 's/ath10k-firmware-qca4019-ct/ath10k-firmware-qca4019/g' {} +
echo "=== 替换完成 ==="
